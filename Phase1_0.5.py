...
class OffboardControl:
    ...
    drop_loc = [84.79, -54.25, 23, 0, 0, 0, 0]
    ...
    def pattern(self):
        ...
        while not rospy.is_shutdown():
            ...
            #     des_x = self.locations[0]
            #     des_y = self.locations[1]
            #     des_z = self.locations[2]
            #     self.des_pose.pose.position.x = des_x
            #     self.des_pose.pose.position.y = des_y
            #     self.des_pose.pose.position.z = des_z
            #     self.des_pose.pose.orientation.x = self.locations[3]
            #     self.des_pose.pose.orientation.y = self.locations[4]
            #     self.des_pose.pose.orientation.z = self.locations[5]
            #     self.des_pose.pose.orientation.w = self.locations[6]       
            ...
            if self.is_ready_to_fly:
                ...
                self.des_pose.pose.position.y = des_y
                self.des_pose.pose.position.z = des_z
                self.des_pose.pose.orientation.x = 0 #self.locations[self.waypointIndex, 3]
                self.des_pose.pose.orientation.y = 0 #self.locations[self.waypointIndex, 4]
                self.des_pose.pose.orientation.z = 0 #self.locations[self.waypointIndex, 5]
                self.des_pose.pose.orientation.w = 0 #self.locations[self.waypointIndex, 6]
                ...
                if dist < self.dist_threshold:
                    ...
                    z_increase = 5 - self.range
                    #print('z_increase',z_increase)

                    mower_ctr += 1

                    #print("X_increase:", x_increase)
                    #print("Y_increase:", y_increase)

                    des_x = self.curr_pose.pose.position.x + x_increase #self.locations[self.waypointIndex, 0]
                    des_y = self.curr_pose.pose.position.y + y_increase #self.locations[self.waypointIndex, 1]
                    des_z = self.curr_pose.pose.position.z + z_increase #self.locations[self.waypointIndex, 2]
    ...
    def flytodestination(self):
        ...
        if self.phase == "SEARCH":
            destination = np.matrix([self.search_loc,self.search_loc])
    ...
    def descent(self):
        ...
        while self.mode == "SWOOP" and self.range > 0.235 and not rospy.is_shutdown():
            #print("X target:",self.destination_x)
            #print("Y target:",self.destination_y)
