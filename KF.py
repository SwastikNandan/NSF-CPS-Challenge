...
class OffboardControl:
    ...
    def rover_odometry(self,data):
        ...
        data.twist.twist.linear = self.rover_linear_velocity
        data.twist.twist.angular = self.rover_angular_velocity
        ...
        multiplication = np.matmul(rot_z_to_global,np.array[self.rover_linvel_x, self.rover_linvel_y, self.rover_linvel_z].T)
        self.rover_linvel_x = multiplication[0][0]
        self.rover_linvel_y = multiplication[1][0]
        self.rover_linvel_z = multiplication[2][0] 
    ...
    def KF(self):
    	...
    	def prediction2d(x, vx, ax, y, vy, ay, z, vz, az):
    		...
    		return X_prime_x, Y_prime_y, Z_prime_z
        ...
        while not rospy.is_shutdown():
        	data_x = self.UTMx - 372232.05
        	data_y = self.UTMy - 4146891.91
        	data_z = self.altitude - 462.785
        	ax = self.rover_linacc_x
        	ay = self.rover_linacc_y
        	az = self.rover_linacc_z
